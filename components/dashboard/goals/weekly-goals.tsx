...
export default function WeeklyGoals() {
  ...
  useEffect(() => {
    ...
    if (
      goals.selectedDays[today] &&
      (!lastVisit || lastVisit.toISOString().split("T")[0] !== todayDate)
    ) {
  ...
  const getProgressMessage = () => {
    ...
    const completedCount = goals.selectedDays.filter(
      (selected, index) => selected && goals.completedDays[index],
    ).length;

    if (selectedCount === 0) return "Set your weekly goals to get started!";

    if (completedCount === 0)
      return "Your week is just beginning. You can do this!";

    if (completedCount === selectedCount)
      return "You are on track. Keep up the excellent work!";
  ...
  return (
    <div className="bg-white rounded-xl p-6 space-y-4 shadow-sm">
      <p className="text-center text-light-300 font-medium">
        {getProgressMessage()}
      </p>
        ...
        {DAYS.map((day, index) => (
          <button
            ...
            className={cn(
              ...
              goals.selectedDays[index] &&
                !goals.completedDays[index] &&
      ...
      <Dialog open={isEditing} onOpenChange={setIsEditing}>
        <DialogTrigger asChild className="items-center">
          <button className="button-bordered !font-bold !px-5 mx-auto block">
            Edit my goal
          </button>
        </DialogTrigger>
        <DialogContent className="bg-white border-none shadow-lg max-w-md p-6">
          ...
          <div className="space-y-6">
            <p className="text-light-300">
              Select the days you want to be active:
            </p>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button
                onClick={saveGoals}
                className="bg-primary hover:bg-primary/90 text-yellow"
              >
                Save
              </Button>
            </div>
